...
// --- ১. ডিফল্ট ইমেজ লোডিং ---
let defaultImgLeft = new Image();
let defaultImgRight = new Image();

// একটি ছোট কালো স্কোয়ার ইমেজ
const defaultAvatarDataUri = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAGQAAABkCAYAAABw4MoLAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAGXRFWHRTb2Z0d2FyZQB3d3cuaW5rc2NhcGUub3Jnm548GgAAADdJREFUeJztwQEBAAAAgiD7pZl5/wsEAAAAYJ8AAAAAAABgoAcAAAAAAABgoAcAAAAAAABgoAcAAAAAAABgoAcAAAB4C/l4AAEoQf10AAAAAElFTkSuQmCC';
defaultImgLeft.src = defaultAvatarDataUri; 
defaultImgRight.src = defaultAvatarDataUri;
...
// ডিফল্ট ইমেজ লোড হওয়ার পর state-এ সেট করুন
let imagesLoaded = 0;
const totalImages = 2;
const checkReady = () => {
    imagesLoaded++;
    if (imagesLoaded === totalImages) {
        state.leftImg = defaultImgLeft;
        state.rightImg = defaultImgRight;
        renderFrame(0); // Initial draw with default images
    }
}
defaultImgLeft.onload = checkReady;
defaultImgRight.onload = checkReady;
...
function loadLeft(e){
  ...
  loadImageFromFile(f, (img)=>{
    if(img){ 
      state.leftImg = img; 
      leftName.textContent = f.name; 
      renderFrame(performance.now()); // নতুন ইমেজ লোড হলে রেন্ডার করুন
    }
...
/* ---------- Recording (MediaRecorder) - সংশোধিত লজিক (২) ---------- */
async function startRecording(){
  // ensure one preview run length
  state.duration = Number(durationSelect.value);
  
  // 30 FPS এ স্ট্রিম ক্যাপচার করুন
  ...
  rec.onstop = ()=> {
    // স্ট্রিম ট্র্যাক বন্ধ করুন (এটি গুরুত্বপূর্ণ)
    ...
    // URL রিসোর্স মুক্ত করুন
    setTimeout(()=> URL.revokeObjectURL(url), 2000);
  };

  // রেন্ডারিং এবং রেকর্ডিং একই সাথে শুরু করুন
  state.playing = true;
  state.t0 = performance.now();
  
  // প্রতি 100ms এ ডেটা উপলব্ধ করার জন্য MediaRecorder কে নির্দেশ দিন (রেকর্ডিংকে স্থিতিশীল করার জন্য)
  rec.start(100); 
  ...
  rafId = requestAnimationFrame(function frame(ts){
    ...
    if(elapsed >= state.duration){
      // নির্ধারিত সময় পূর্ণ হলে বন্ধ করুন
      state.playing = false;
      
      // সামান্য বিলম্ব নিশ্চিত করে যে শেষ ফ্রেমটি MediaRecorder ক্যাপচার করেছে।
      setTimeout(() => {
          rec.stop();
      }, 50); 
...
// ডিফল্ট ইমেজ লোড না হওয়া পর্যন্ত অপেক্ষা করুন
// Initial draw এখন checkReady() কল করবে।
</script>
</body>
</html>
